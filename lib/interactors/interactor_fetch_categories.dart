...
class InteractorFetchCategories {
  Future requestAllCategories(LibraryProvider provider) async {
    ...
    categories
        .addAll(await getHomeCategories().then((categories) => categories));

    provider.categories.value = categories;
  }

  Future setHomeCategories(LibraryProvider provider) async {
    final categories = await getHomeCategories();
    provider.homeCategories.value = [...categories];
  }
}
